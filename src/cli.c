...
static int theme_bg(Theme theme) { return theme == THEME_DARK ? 0x0c0c0c : 0xf5f5f0; }
static int theme_fg(Theme theme) { return theme == THEME_DARK ? 0x33ff33 : 0xb35900; }
...
static int theme_dim(Theme theme) { return theme == THEME_DARK ? 0x888888 : 0x666666; }

static void line_append(Terminal *t, const char *text, int fg) {
...
static void line_wrap_append(Terminal *t, const char *text, int fg) {
    if (t->cols <= 0) return;
    const char *p = text;
...
static void cmd_execute(Terminal *t, const char *raw) {
    ...
    if (strcmp(cmd, "help") == 0) {
        line_wrap_append(t, "Available commands:", theme_prompt(t->theme));
        line_wrap_append(t, "  help      - show this help", theme_fg(t->theme));
        line_wrap_append(t, "  about     - about this demo", theme_fg(t->theme));
        line_wrap_append(t, "  projects  - list sample projects", theme_fg(t->theme));
        line_wrap_append(t, "  skills    - tech skills snapshot", theme_fg(t->theme));
        line_wrap_append(t, "  contact   - how to reach out", theme_fg(t->theme));
        line_wrap_append(t, "  theme     - toggle color theme", theme_fg(t->theme));
        line_wrap_append(t, "  clear     - clear the screen", theme_fg(t->theme));
    } else if (strcmp(cmd, "about") == 0) {
        line_wrap_append(t, "cweb - a pure C / WebAssembly terminal demo.", theme_fg(t->theme));
        line_wrap_append(t, "Everything you see is rendered from C, compiled with Emscripten,", theme_dim(t->theme));
        line_wrap_append(t, "and drawn to an HTML5 canvas. No JavaScript frameworks involved.", theme_dim(t->theme));
    } else if (strcmp(cmd, "projects") == 0) {
        line_wrap_append(t, "* cweb (this site) - C + WASM CLI portfolio", theme_fg(t->theme));
        line_wrap_append(t, "* tiny-term - minimal VT100 emulator", theme_dim(t->theme));
        line_wrap_append(t, "* wasm-2048 - 2048 clone in C for the browser", theme_dim(t->theme));
    } else if (strcmp(cmd, "skills") == 0) {
        line_wrap_append(t, "Languages: C, JavaScript, Python", theme_fg(t->theme));
        line_wrap_append(t, "Tools:    Emscripten, Make, GitHub Actions", theme_fg(t->theme));
        line_wrap_append(t, "Focus:    systems, embedded, web assembly", theme_fg(t->theme));
    } else if (strcmp(cmd, "contact") == 0) {
        line_wrap_append(t, "Email: hello@example.com", theme_fg(t->theme));
        line_wrap_append(t, "GitHub: github.com/example", theme_fg(t->theme));
    } else if (strcmp(cmd, "theme") == 0) {
        t->theme = (t->theme == THEME_DARK) ? THEME_LIGHT : THEME_DARK;
        line_wrap_append(t, t->theme == THEME_DARK ? "Switched to dark theme." : "Switched to light theme.", theme_prompt(t->theme));
    } else if (strcmp(cmd, "clear") == 0) {
        t->line_count = 0;
    } else {
        char err[CLI_MAX_LINE_LEN];
        snprintf(err, sizeof(err), "%s: command not found. Try 'help'.", cmd);
        line_wrap_append(t, err, theme_error(t->theme));
    }
}
...
void cli_init(Terminal *t) {
    ...
    line_wrap_append(t, "Welcome to cweb. Type 'help' to get started.", theme_prompt(t->theme));
}
...
void cli_handle_key(Terminal *t, int key_code) {
    if (key_code == 8) { // Backspace
        ...
        line_append(t, full, theme_prompt(t->theme));
...
void cli_render(Terminal *t) {
    ...
    for (int i = start; i < t->line_count; i++) {
        br_draw_text(0, y, t->lines[i].text, t->lines[i].fg, t->line_height);
        y += t->line_height;
    }
    ...
    if (t->cursor_blink) {
        ...
        br_draw_rect(cx, cy, t->char_width - 1, t->line_height - 4, theme_fg(t->theme));
    }
}
